    ...
    fn: (msg) =>{
        ...
        try{
            ...
            if (msg.author.id === msg.guild.ownerID || msg.member.permissions.has(PermissionsBitField.Flags.Administrator)) {
                ...
                if(!args[1]){
                    return msg.reply({content: '指令格式錯誤，此指令格式為`!=RemoveLeaveImg 圖片網址`，請重新輸入'})
                };
                ...
                imgUrl.get().then(res => {
                    ...
                    if(res.exists){
                        imgUrl.update({
                            LeaveImg:admin.firestore.FieldValue.arrayRemove(args[1])
                        })
                        
                        .then(
                            msg.reply({ content: '已刪除離開訊息圖片： ' + args[1]}),
                            console.log('Delete Leave Img Data Success: ' + args[1])
                        );
                    };
                    return
                })
