    ...
    fn: (msg) =>{
        ...
        try{
            ...
            if (msg.author.id === msg.guild.ownerID || msg.member.permissions.has(PermissionsBitField.Flags.Administrator)) {
                ...
                if(!args[1]){
                    return msg.reply({content: '指令格式錯誤，此指令格式為`!=AddLeaveImg 圖片網址`，請重新輸入'})
                };
                ...
                imgUrl.get().then(res => {
                    if(!res.exists){
                        imgUrl.set({
                            LeaveImg: admin.firestore.FieldValue.arrayUnion(args[1])
                        })

                        .then(
                            console.log("Set new Leave Img success: " + args[1])
                        );
                    }
                ...
                return msg.reply({ content: `新增離開圖： ${args[1]}` });
