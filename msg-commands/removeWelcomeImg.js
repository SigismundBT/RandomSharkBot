    ...
    fn: (msg) =>{
        ...
        try{
            ...
            if (msg.author.id === msg.guild.ownerID || msg.member.permissions.has(PermissionsBitField.Flags.Administrator)) {
                ...
                if(!args[1]){
                    return msg.reply({content: '指令格式錯誤，此指令格式為`!=RemoveWelcomeImg 圖片網址`，請重新輸入'})
                };
                ...
                imgUrl.get().then(res => {
                    ...
                    if(res.exists){
                        imgUrl.update({
                            WelcomeImg:admin.firestore.FieldValue.arrayRemove(args[1])
                        })
                        
                        .then(
                            msg.reply({ content: '已刪除加入訊息圖片： ' + args[1]}),
                            console.log('Delete Welcome Img Data Success: ' + args[1])
                        );
                    };
                    return
                })
